...
export class CategoryService {
    ...
    getAttributes() {
        ...
        return new Promise(resolve => {
            this.http.get(this.config.setUrl('GET', '/wp-json/wc/v2/products/attributes?', {per_page: 100}), this.config.optionstwo).map(res => res.json()).subscribe(data => {
    ...
    attributeTerms(id) {
        ...
        return new Promise(resolve => {
            this.http.get(this.config.setUrl('GET', '/wp-json/wc/v2/products/attributes/' + id + '/terms?', {per_page: 100}), this.config.optionstwo).map(res => res.json()).subscribe(data => {
