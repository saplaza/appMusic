...
export class Service {
    ...
    load() {
        this.http.get(this.config.setUrl('GET', '/wp-json/wc/v2/products/?'), this.config.optionstwo).map(res => res.json()).subscribe(data => {
            this.products = data;
        });
        return new Promise(resolve => {
            this.http.get(this.config.url + '/wp-admin/admin-ajax.php?action=mstoreapp-keys', this.config.options).map(res => res.json()).subscribe(data => {
                ...
                this.nativeStorage.setItem('blocks', data).then(data => console.log('Saved'), error => console.error(error));
                for (let item in this.blocks) {
                    if (this.blocks[item].block_type == 'product_block') {
                        this.http.get(this.config.setUrl('GET', '/wp-json/wc/v2/products/?', {
    ...
    getAddress() {
        return new Promise(resolve => {
            this.http.get(this.config.setUrl('GET', '/wp-json/wc/v2/customers/' + this.values.customerId + '?'), this.config.optionstwo).map(res => res.json()).subscribe(data => {
                this.address = data;
    ...
    saveAddress(address) {
        var params = address;
        return new Promise(resolve => {
            this.http.put(this.config.setUrl('PUT', '/wp-json/wc/v2/customers/' + this.values.customerId + '?'), params, {withCredentials: false}).map(res => res.json()).subscribe(data => {
                resolve(data.data);
            }, err => {
                resolve(JSON.parse(err._body));
            });
    ...
    getOrder(id) {
        return new Promise(resolve => {
            this.http.get(this.config.setUrl('GET', '/wp-json/wc/v2/orders/' + id + '?'), this.config.optionstwo).map(res => res.json()).subscribe(data => {
                this.order = data;
                resolve(this.order);
            });
        });
    ...
    registerCustomer(customer) {
        var params = customer;
        return new Promise(resolve => {
            this.http.post(this.config.setUrl('POST', '/wp-json/wc/v2/customers?'), params, {withCredentials: false}).map(res => res.json()).subscribe(data => {
                this.user = data;
                resolve(this.user);
            }, err => {
                resolve(JSON.parse(err._body));
    ...
    updateOrder(data, id) {
        return new Promise(resolve => {
            this.http.put(this.config.setUrl('PUT', '/wp-json/wc/v2/orders/' + id + '?'), data, {withCredentials: true}).map(res => res.json()).subscribe(data => {
                this.status = data;
                resolve(this.status);
            }, err => {
                resolve(JSON.parse(err._body));
    ...
    getbestOffers() {
        ...
        let filter = {status: 'publish', on_sale: true, category: 0};
        for (let item in this.mainCategories) {
            filter.category = this.mainCategories[item].id;
            this.http.get(this.config.setUrl('GET', '/wp-json/wc/v2/products/?', filter), this.config.optionstwo).map(res => res.json()).subscribe(data => {
